...
std::tuple<float, float, float> Satellite_Processor::get_position(size_t idx){
    return get_position_after(idx, 0);
}

std::tuple<float, float, float> Satellite_Processor::get_position_after(size_t idx, U32 dur){
    auto elapsed_time = (get_elapsed_time()+ms(dur)).count();
...
void Satellite_Processor::populate(U16 amount = 0xFFFF) {
    ...
    for(U16 i = 0; i < amount; i++){
        ...
        // Generate a randomized unit vector, then scale it by LEO_ALTITUDE
        // OPTIONAL TODO: if threads isn't full, designate threads to do this
        // Polar coordinate:
        /*
        x: LEO_ALTITUDE * cos(random_theta) * sin(random_phi)
        y: LEO_ALTITUDE * sin(random_theta) * sin(random_phi)
        z: LEO_ALTITUDE * cos(random_phi)
        */
        ...
        float x = LEO_ALTITUDE * std::cos(random_theta) * sin_phi, 
        y = LEO_ALTITUDE * std::sin(random_theta) * sin_phi, 
        z = LEO_ALTITUDE * cos_phi;
        ...
        // z * T_x - x * T_z
        // x * T_y - y * T_x
        // ^ Axis of rotation for the satellite
